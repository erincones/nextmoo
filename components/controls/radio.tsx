    ...
    <div className={className}>
      ...
      <label htmlFor={id} onClick={handleLabelClick}>
        <span className="cursor-pointer">
          (<button ref={button} type="button" tabIndex={0} onClick={handleChange} aria-role="radio" aria-checked={checked ? `true` : `false`} aria-label={children} className="whitespace-pre focus:bg-white focus:text-black focus:outline-none">{checked ? `*` : ` `}</button>)
        </span>
        {children}
      </label>
    </div>
