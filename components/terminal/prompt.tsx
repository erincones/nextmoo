 ...
 */
export const rawPrompt = ({
  user = initial.user,
  host = initial.host,
  path = initial.path,
}: PromptData): string =>
  user === `root` ? `[${host} ${path}]# ` : `[${user}@${host} ${path}]$ `;
 ...
 */
export const Prompt = forwardRef<HTMLPreElement, PromptProps>(
  (
    {
      id,
      user = initial.user,
    ...
    return (
      <pre ref={ref} id={id} className={className}>
        {content}
        {children}
      </pre>
    );
